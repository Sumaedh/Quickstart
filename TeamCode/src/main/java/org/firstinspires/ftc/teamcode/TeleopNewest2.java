...
public class TeleopNewest2 extends OpMode {
    ...
    public void init() {
        ...
        turretController = new PIDController(pTurret, iSTurret, dTurret);
    ...
    public void loop() {
        ...
        turretController.setPID(pTurret, iSTurret, dTurret);
        double turretError = 0 - lastTx;

        double pidOutputTurret = turretController.calculate(lastTx, 0);
        double feedforward = Math.copySign(kFFTurret, turretError);
        double motorPowerTurret = pidOutputTurret + feedforward;

        motorPowerTurret = Math.max(-1.0, Math.min(1.0, motorPowerTurret));

        rotationMotor.setPower(motorPowerTurret);
    }
}
